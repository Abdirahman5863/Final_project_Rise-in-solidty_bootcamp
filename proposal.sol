...
contract ProposalContract {
    ...
    function vote(uint8 choice) external active newVoter(msg.sender) {
        ...
        proposal.current_state = calculateCurrentState(proposal);
    ...
    function calculateCurrentState(Proposal memory proposal) private pure returns(bool) {
        uint256 pass = proposal.pass / 2;

        if (proposal.approve > proposal.reject + pass) {
            return true;
        } else {
            return false;
        }
